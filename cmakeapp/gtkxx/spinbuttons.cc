...
public:
  SpinButtons()
      : m_Frame_NotAccelerated("Not accelerated"),
        ...
        m_adjustment_day(
            Gtk::Adjustment::create(1.0, 1.0, 31.0, 1.0, 5.0, 0.0)),
        m_adjustment_month(
            Gtk::Adjustment::create(1.0, 1.0, 12.0, 1.0, 5.0, 0.0)),
        m_adjustment_year(
            Gtk::Adjustment::create(2012.0, 1.0, 2200.0, 1.0, 100.0, 0.0)),
        m_adjustment_value(
            Gtk::Adjustment::create(0.0, -10000.0, 10000.0, 0.5, 100.0, 0.0)),
        m_adjustment_digits(
            Gtk::Adjustment::create(2.0, 1.0, 5.0, 1.0, 1.0, 0.0)),
        ...
        m_Button_Int("Value as Int"), m_Button_Float("Value as Float"),
        m_Button_Close("Close") {

    set_title("Spin Buttons");
    ...
    m_SpinButton_Year.set_size_request(80, -1);
    ...
    m_adjustment_digits->signal_value_changed().connect(
        sigc::mem_fun(*this, &SpinButtons::on_spinbutton_digits_changed));
    ...
    m_CheckButton_Snap.signal_toggled().connect(
        sigc::mem_fun(*this, &SpinButtons::on_checkbutton_snap));

    m_VBox_Accelerated.append(m_CheckButton_Numeric);
    m_CheckButton_Numeric.set_expand();
    m_CheckButton_Numeric.set_active();
    m_CheckButton_Numeric.signal_toggled().connect(
        sigc::mem_fun(*this, &SpinButtons::on_checkbutton_numeric));

    // Buttons
    m_VBox_Accelerated.append(m_HBox_Buttons);

    m_Button_Int.signal_clicked().connect(
        sigc::bind(sigc::mem_fun(*this, &SpinButtons::on_button_getvalue),
                   VALUE_FORMAT_INT));
    ...
    m_Button_Close.signal_clicked().connect(
        sigc::mem_fun(*this, &SpinButtons::on_button_close));
...
protected:
  ...
  enum enumValueFormats { VALUE_FORMAT_INT, VALUE_FORMAT_FLOAT };
  void on_button_getvalue(enumValueFormats display) {
    ...
    if (display == VALUE_FORMAT_INT)
      ...
      sprintf(buf, "%.*f", m_SpinButton_Value.get_digits(),
  ...
  Gtk::Box m_HBox_NotAccelerated, m_HBox_Accelerated, m_HBox_Buttons;
  Gtk::Box m_VBox_Main, m_VBox, m_VBox_Day, m_VBox_Month, m_VBox_Year,
      m_VBox_Accelerated, m_VBox_Value, m_VBox_Digits;
  Gtk::Label m_Label_Day, m_Label_Month, m_Label_Year, m_Label_Value,
      m_Label_Digits, m_Label_ShowValue;
  Glib::RefPtr<Gtk::Adjustment> m_adjustment_day, m_adjustment_month,
      m_adjustment_year, m_adjustment_value, m_adjustment_digits;
...
int main(int argc, char *argv[]) {
  ...
  return app->make_window_and_run<SpinButtons>(argc, argv);
}
